...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('armsArmoury.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('armsArmoury.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('armsArmoury.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
